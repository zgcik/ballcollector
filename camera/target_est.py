...
def target_pose_est(int_matrix, detection, rob_pose):
    ...
    # NOTE: x,y are bbox CENTER not top-left (docs are wrong,this has been tested)
    ...
    target_pose = {
        "x": (rob_pose[1] + delta_y_world),
        "y": (rob_pose[0] + delta_x_world),
    }
...
def merge_ests(detections):
    ...
    labels = dbscan.fit_predict(points)  # type: ignore
    j = 0
