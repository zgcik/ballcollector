...
class TargetEst:
        def __init__(self):
                self.target_dims = {'Balls': 0.0342*2}
                self.detections = []
        ...
        def target_pose_est(self, int_matrix, detection, rob_pose):
                ...
                true_height = self.target_dims[target_class][2]
                ...
                self.detections.append(target_pose)
        
        def merge_ests(self, target_dict):
                target_est = []
                points = [[point['x'], point['y']] for point in self.detections.values()]
