...
class Robot:
    def __init__(self, wheel_radius=0.028, base=0.22):
        ...
        self.motor1 = Motor(enable1=27, enable2=17, pwm1=18, encoder1=5, encoder2=6, encoderppr=self.encoder_ticks)  # Right Motor
        self.motor2 = Motor(enable1=22, enable2=23, pwm1=19, encoder1=12, encoder2=13, encoderppr=self.encoder_ticks)  # Left Motor
    ...
    def rotate(self, angle_diff):
        ...
        if abs(angle_diff) > 0.01:  # Threshold to avoid tiny adjustments
            ...
            encoder_steps_needed = self.calculate_encoder_steps(arc_length)/2
            ...
            self.motor1._encoder.steps = 0
            self.motor2._encoder.steps = 0
            ...
            total_steps_motor_1 = total_steps_motor_2 = 0
            current_steps_motor=0
            previous_steps_motor=0

            # Rotate until the required number of encoder steps is reached
            while (total_steps_motor_1 < encoder_steps_needed) and (total_steps_motor_2 < encoder_steps_needed):
                ...
                if angle_diff > 0:
                    motor_steps_1 = abs(self.motor1._encoder.steps)
                    motor_steps_2 = abs(self.motor2._encoder.steps)
                    #PID update
                ...
                total_steps_motor_1 = motor_steps_1
                total_steps_motor_2 = motor_steps_2

                # print(f'total_steps_motor_1 {total_steps_motor_1}')
                # print(f'total_steps_motor_2 {total_steps_motor_2}')
    ...
    def drive_forward(self, distance):
        ...
        if distance > 0.01:  # Threshold to avoid unnecessary movements
            ...
            while (total_steps_motor2  < encoder_steps_needed) and (total_steps_motor1 < encoder_steps_needed):
                ...
                #print(f"Actual Motor 1 PWM: {motor_1_new_pwm}")
                # print(f"Actual Motor 2 PWM: {motor_2_new_pwm}")
...
class PIDController:
    def __init__(self, Kp, Ki, Kd, pwm, output_limits=(0, 1), dt=0.1, encoder=0):
        ...
        self.Kd = Kd
        self.pwm = pwm
        self.setpoint = (self.rpm * self.pwm *self.encoder_ticks)/(100) #(RPM_max * PWM)/100 *PPR
...
if __name__ == "__main__":
    robot = Robot()
    # Drive to (0.5 meter, 0 meter)
    
    # robot.drive_to_location([0.7716184159762127,-0.06112025907605417])

    robot.drive_to_location([0.5,0])
    
    time.sleep(2)

    robot.drive_to_location([0.5,0.5])

    #robot.drive_to_location([0,0.5])
